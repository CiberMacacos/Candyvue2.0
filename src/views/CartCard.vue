...
<template>
  <div class="w-full flex justify-around items-center text-center my-3 sm:p-4 md:p-6 md:gap-4 lg:gap-46 ">
    <div class="flex flex-col gap-3 justify-center hidden sm:block">
      <h1 class="font-bold sm:text-sm ">Productos</h1>
      <img class="w-12 md:w-28" :src="image" :alt="name">
    </div>
    <div class="flex flex-col gap-3 items-center text-center w-12">
      <h1 class="font-bold text-sm md:text-base">Nombre</h1>
      <h2 class="text-sm sm:text-sm md:text-lg lg:text-xl sm:gap4">{{ name }}</h2>
    </div>
    <div class="flex flex-col gap-3 justify-center items-center hidden sm:block">
      ...
      <p class="text-sm sm:text-sm md:text-lg lg:text-xl">{{ price }}€</p>
    </div>
    <div class="flex flex-col gap-3 items-center">
      ...
      <div class="flex flex-row justify-center items-center lg:gap-6 md:gap-4 sm:gap-4 gap-2">
        ...
        <p class="text-sm sm:text-sm md:text-lg lg:text-xl">{{ quantity }}</p>
    ...
    <div class="flex flex-col gap-3">
      ...
      <h2 class="text-sm sm:text-sm md:text-lg lg:text-xl">{{ total }}€</h2>
    </div>
    <div class="flex flex-col justify-center items-center">
      <Popper hover content="Eliminar producto">
        <button @click="this.removeProduct({ id })">
          <img class="w-5 sm:w-8 md:w-10" src="/assets/icon/basura.png" alt="papelera">
        </button>
      </Popper>
    </div>
  </div>
