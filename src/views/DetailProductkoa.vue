...
<template>
  ...
  <div class="sm:w-full md:w-full flex-col items-center inline-flex justify-center md:gap-5 mb-9" id="container">
    ...
    <div class="justify-center">
      <p class="mb-3 text-sm md:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 via-cyan-600 to-purple-600">{{ this.data.name }}</p>
      <p class="mb-3 text-sm md:text-2xl text-black text-center">{{ this.data.price }} €</p>
    </div>
    <div class="bg-blue-50 w-auto md:p-4 flex flex-col items-center border-blue-800 max-w-xl hover:bg-pink-50 gap-5">
      ...
      <Popper hover content="Añadir producto al carrito">
        <button @click="this.addProduct(this.data)"><img class="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 cursor-pointer hover:invert" src="/assets/icon/carrito-de-compra-anadir.png" alt="Comprar"></button>
      </Popper>
    </div>
  </div>
  ...
  <div
    ...
    <h1 class="text-white text-xs sm:text-sm md:text-xl lg:text-4xl text-center md:w-72 lg:w-96 ml-48 md:ml-88 lg:ml-96 mr-1 lg:mt-3">¡Síguenos en nuestras redes y no te pierdas las novedades!</h1>
